...
extern PubSubClient MQTTclient;
//#include <version.h>
//extern char publishMoistureStateTopic[];
//extern char publishMoistureLevelTopic[];
char publishMoistureStateTopic[] = "Plant/3/MoistureState";
char publishMoistureLevelTopic[] = "Plant/3/MoistureLevel";
void checkMoistureSensor()
{
    char str[21];

    myMoistureSensor.getLevel(); // trigger sampling if due
    if (myMoistureSensor.hasNewLevel())
    {
        MQTTclient.publish(publishMoistureStateTopic, myMoistureSensor.getState() ? "true" : "false");
        sprintf(str, "%d", myMoistureSensor.getLevel());
        MQTTclient.publish(publishMoistureLevelTopic, str);
        Serial.print("mqtt=>: ");
        Serial.println(str);
